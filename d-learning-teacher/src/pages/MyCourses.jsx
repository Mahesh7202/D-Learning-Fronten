...
import Services from "../services/TeacherServices";
...
const MyCourses = () => {
  ...
  const search = async () => {
    ...
    const student = await StudentService.getTeacherByEmcode(localStorage.getItem("userId"));
    return student.data["department"] + "" + student.data["branch"] + "" + student.data["semno"];
  }

  const loadUser = async () => {

    const teacher = await Services.getTeacherByEmcode("2201k1953hs");
    ...
    const courses = await Services.getTeacherCoursesByEmcode(teacher.data.emcode);
console.log(courses.data.courses);
    ...
    const getName = (a) => {
      const res = coursedata.data.filter((row) => {
        return row.sucode == a
      });
      return res.length > 0 ? res[0].coursename : ''
    }

    const data = courses.data.courses.map((a, index) =>
    (
      {
        id: index + 1,
        sucode: a,
        coursename: getName(a),
        resource: getRes(a)
      }));
